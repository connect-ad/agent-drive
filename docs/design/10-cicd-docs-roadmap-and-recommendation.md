# AgentDrive — CI/CD, Documentation Plan, Roadmap, ADRs & Final Recommendation
...
jobs:
  ...
  preview-deploy:
    ...
    steps:
      ...
      - run: npm run test:e2e -- --base-url=https://preview.agentdrive.dev
```
...
jobs:
  ...
  deploy:
    ...
    runs-on: ubuntu-latest
    steps:
      - run: npx wrangler d1 migrations apply agentdrive-db --env production
      - run: npx wrangler deploy --env production
      - run: npx wrangler pages deploy apps/web/dist --project-name=agentdrive-app
      - name: Smoke test
        run: npm run test:smoke -- --base-url=https://api.agentdrive.dev
...
|---|---|---|
| `README.md` | What AgentDrive is, quickstart (create a workspace, get a key, make your first API call), links to everything else | Hand-written, kept short |
