# AgentDrive — Authentication, Security, Privacy & Legal Copy
...
**Human authentication (MVP-0): email + password, plus a passwordless magic-link option.** **PROPOSAL — reasoning.** AgentStorage shipped GitHub OAuth from day one (PART 2.6); we deliberately sequence differently: email/password (bcrypt/Argon2id-hashed, PART 16.5) covers 100% of the target developer persona with zero third-party app-registration lead time, and a magic-link option (single-use, 15-minute-expiry emailed token) covers users who'd rather not manage a password at all — both ship in MVP-0. **GitHub OAuth is added in MVP-1**, once the core loop is validated, not because OAuth is unimportant but because it's the kind of feature that benefits from being added deliberately rather than rushed into an MVP-0 timeline (OAuth app review, callback URL management per environment, PART 18). Google OAuth is a V2 candidate, added only if signup-funnel data shows real demand — we do not build it speculatively.

**Why not passwordless-only:** a password remains useful as a fallback when email delivery is delayed or a magic link expires mid-flow, and some developers actively prefer a password manager over an email round-trip — offering both, not forcing one, is the better developer experience.
...
**Session security (humans):** short-lived (15-minute) signed JWT access token + a longer-lived (30-day) httpOnly, Secure, SameSite=Lax refresh token stored as a cookie, rotated on every use (refresh-token rotation — reuse of an already-rotated token invalidates the whole session family, a standard defense against stolen-refresh-token replay). No session state lives in D1 for the access token itself (stateless, verified by signature) — only the refresh token family is tracked, in the `refresh_tokens` table (`05` PART 11.1/11.1a), to support the "active sessions" list and remote revocation UI (PART 8.23). Magic-link and password-reset tokens are a separate, deliberately lighter-weight mechanism — single-use, short-TTL entries in KV rather than a D1 table (`05` PART 11.1a) — since they need no relational query capability and benefit from KV's automatic expiry.
...
**Authorization middleware runs before business logic, always — no exceptions.** Every request (REST or MCP) passes through, in order: (1) authenticate (resolve session or API key to an identity), (2) load the identity's effective scope (role or key scopes), (3) resolve the target resource's `workspace_id`, (4) check the identity's scope against the requested operation and path, (5) check workspace quota if the operation is quota-relevant, and only then (6) invoke the handler. Every handler function receives an already-authorized, already-workspace-scoped context object — a handler is structurally unable to query outside the caller's workspace, because it never receives a raw, unscoped database client (PART 16.4 shows the concrete pattern).
...
Covered in full in PART 15. Security-relevant additions: all authentication endpoints (`/login`, `/signup`, `/forgot-password`) are rate-limited independent of the general API rate limits (16.14) specifically to blunt credential-stuffing and brute-force, and return **generic, non-enumerating error messages** ("That email or password isn't right" — never "no account with that email" or "incorrect password" as distinguishable messages) to prevent account enumeration (PART 8.5/8.7 already encode this into the UI copy).
...
**Argon2id** (preferred; bcrypt as a fallback if the Workers runtime environment lacks a suitable Argon2 implementation at build time — confirmed at implementation time, not assumed) with a per-password random salt, workspace-appropriate cost parameters tuned to ~250ms verification time on Workers' CPU budget. Minimum 8 characters enforced client- and server-side (matching the UI copy in PART 8.3); no maximum-length-driven truncation bugs (cap input at a generous 256 chars to prevent a hashing-cost DoS from a client submitting a multi-megabyte "password"). No composition rules beyond length (no forced special-character rules — modern guidance, e.g. NIST 800-63B, favors length and breach-checking over composition complexity). **V2 candidate:** checking new passwords against the HaveIBeenPwned range API (k-anonymity, no plaintext password ever leaves the server) — not in MVP-0/1 to keep the auth dependency surface minimal at launch, added once there's bandwidth to test it properly.

### 16.6 Session Security
Covered in PART 15.1. Additional: the refresh-token cookie is scoped to the API's own domain only (not a shared parent domain across dashboard+API unless a same-site architecture requires it — PART 18 domain design keeps `app.` and `api.` on the same registrable domain specifically so cookies can be scoped tightly without cross-subdomain leakage risk).

### 16.7 CSRF
The dashboard's browser-facing endpoints (anything relying on the httpOnly session cookie) require a CSRF token: a random value set in a **readable** (non-httpOnly) cookie at login and mirrored by the frontend into a custom request header (`X-CSRF-Token`) on every mutating request; the Worker rejects any state-changing request where the header doesn't match the cookie (the standard double-submit pattern). `SameSite=Lax` on the session cookie itself is a second, independent layer (blocks the cookie from being sent on most cross-site subrequests in the first place). **Agent/API-key-authenticated requests are exempt from CSRF checks** — they use a bearer token in a header, never a cookie, so there's no ambient credential for a malicious page to ride on.

### 16.8 CORS
The dashboard origin (`app.agentdrive.dev`) is the only origin allowed to make cookie-authenticated cross-origin requests to the API; `Access-Control-Allow-Credentials: true` is set only for that exact origin, never a wildcard. Bearer-token (API key) requests are **not** subject to the browser's CORS-credentialed restriction in the same way, but CORS headers for the public API still default to `*` for `GET` requests without credentials (so a developer can call the API from a browser-based tool using their own key) while any cookie-based request from an unrecognized origin is rejected outright.
...
- Body: "We use essential cookies to keep you signed in. With your permission, we'd also like to use analytics cookies to understand how AgentDrive is used — never to sell your data."
...
> **This is a first draft prepared to accompany a product design document. It is not legal advice, has not been reviewed by a lawyer, and must not be published as-is. A qualified privacy/data-protection attorney should review this against the jurisdictions AgentDrive actually operates in and the sub-processors actually in use before launch.**

**AgentDrive Privacy Policy**
*Last updated: [DATE] — Draft, pending legal review*

**1. Introduction.** This Privacy Policy explains how AgentDrive ("we," "us") collects, uses, and shares information when you use our website, dashboard, API, and MCP server (together, the "Service"). It applies to human account holders and to information generated by AI agents acting under an account holder's authorization.
...
**21. Contact.** Questions about this policy: [privacy@agentdrive.dev — placeholder]. **[Legal review required: confirm the correct legal entity name, registered address, and any required Data Protection Officer designation before publication.]**
...
**18. Contact.** [legal@agentdrive.dev — placeholder].
